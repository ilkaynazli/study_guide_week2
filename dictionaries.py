...
def get_sum_zero_pairs(numbers):
    ...
    while num1 <= num2:
        if sorted_numbers[num1] + sorted_numbers[num2] == 0:
            result.append([sorted_numbers[num1],sorted_numbers[num2]])
            num1 += 1
            num2 -= 1
        elif sorted_numbers[num1] + sorted_numbers[num2] > 0:
            num2 -=1
        else:
            num1+=1
...
def top_chars(phrase):
    ...
    sorted_character_dict = sorted(character_dict.items(), 
                            key=lambda my_tuple: my_tuple[1], reverse=True)

    top_characters = [sorted_character_dict[0][0]]

    for i in range(len(sorted_character_dict)-1):
        if sorted_character_dict[0][1] >= sorted_character_dict[i+1][1]:
            top_characters.append(sorted_character_dict[i+1][0])
